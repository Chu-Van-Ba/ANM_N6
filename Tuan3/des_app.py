...
#         self.root = root
#         self.root.title("Ứng dụng mã hóa và giải mã dữ liệu theo chuẩn DES")
...
#         # Chỉ set minsize cố định, không đo và lock theo kích thước thực tế
#         self.root.minsize(950, 600)
#         self.root.geometry("1100x700")   # kích thước mặc định khi mở

#     def _build_ui(self):
#         title_bar = tk.Frame(self.root, bg="#1A237E", pady=8)
#         title_bar.pack(fill=tk.X)
#         tk.Label(title_bar, text="🔐  HỆ MẬT MÃ DES",
#                 font=("Segoe UI", 16, "bold"), fg="white", bg="#1A237E").pack()

#         main = tk.Frame(self.root, bg="#F0F4F8", padx=10, pady=10)
#         main.pack(fill=tk.BOTH, expand=True)

#         main.columnconfigure(0, weight=1, minsize=200)  # giảm minsize
...
#         frame = self._panel(parent, "🔑  TẠO KHÓA", "#E3F2FD", col=0)

#         tk.Label(frame, text="Chế độ nhập khóa:", bg="#E3F2FD",
#                  font=("Segoe UI", 10, "bold")).pack(anchor="w")
#         rb_frame = tk.Frame(frame, bg="#E3F2FD")
#         rb_frame.pack(anchor="w")
#         tk.Radiobutton(rb_frame, text="Thủ công", variable=self.key_mode, value="manual",
#                        bg="#E3F2FD", font=("Segoe UI", 10),
#                        command=self._on_key_mode_change).pack(side=tk.LEFT)
#         tk.Radiobutton(rb_frame, text="Tự động", variable=self.key_mode, value="auto",
#                        bg="#E3F2FD", font=("Segoe UI", 10),
#                        command=self._on_key_mode_change).pack(side=tk.LEFT)

#         tk.Label(frame, text="Khóa (16 ký tự HEX):", bg="#E3F2FD",
#                  font=("Segoe UI", 10, "bold")).pack(anchor="w", pady=(8,0))
#         self.key_entry = tk.Text(frame, height=2, font=("Courier New", 11),
#                                  relief=tk.SOLID, bd=1, wrap=tk.WORD)
#         self.key_entry.pack(fill=tk.X, pady=4)

#         self.btn_gen = self._btn(frame, "🎲  Sinh khóa ngẫu nhiên", self._generate_key,
#                                  bg="#1976D2", fg="white")
#         self._btn(frame, "🔄  Reset", self._reset_key, bg="#F9A825", fg="white")
#         self._btn(frame, "📋  Chuyển khóa", self._copy_key_to_decrypt, bg="#E91E63", fg="white")

#         tk.Label(frame, text="(Khóa dùng chung cho mã hóa và giải mã)",
#                  bg="#E3F2FD", font=("Segoe UI", 8), fg="#555").pack(pady=(6,0))

#     # ---------- ENCRYPT PANEL ----------
#     def _build_encrypt_panel(self, parent):
#         frame = self._panel(parent, "🔒  MÃ HÓA DỮ LIỆU", "#E8F5E9", col=1)

#         tk.Label(frame, text="Bản rõ:", bg="#E8F5E9",
#                  font=("Segoe UI", 10, "bold")).pack(anchor="w")
#         self.plain_text = tk.Text(frame, height=6, font=("Segoe UI", 10),
#                                   relief=tk.SOLID, bd=1, wrap=tk.WORD)
#         self.plain_text.pack(fill=tk.X, pady=4)

#         bf = tk.Frame(frame, bg="#E8F5E9")
#         bf.pack(fill=tk.X, pady=2)
#         self._btn(bf, "📂  Mở File", self._open_plain_file, bg="#388E3C", fg="white", side=tk.LEFT)
#         self._btn(bf, "❌  Xóa", lambda: self.plain_text.delete("1.0", tk.END),
#                   bg="#B71C1C", fg="white", side=tk.LEFT)

#         self._btn(frame, "🔒  Mã hóa", self._encrypt, bg="#2E7D32", fg="white")

#         tk.Label(frame, text="Bản mã (HEX):", bg="#E8F5E9",
#                  font=("Segoe UI", 10, "bold")).pack(anchor="w", pady=(6,0))
#         self.cipher_text = tk.Text(frame, height=5, font=("Courier New", 9),
#                                    relief=tk.SOLID, bd=1, wrap=tk.WORD, state=tk.DISABLED)
#         self.cipher_text.pack(fill=tk.X, pady=4)

#         bf2 = tk.Frame(frame, bg="#E8F5E9")
#         bf2.pack(fill=tk.X, pady=2)
#         self._btn(bf2, "➡  Chuyển sang bản mã", self._transfer_cipher,
#                   bg="#1565C0", fg="white", side=tk.LEFT)
#         self._btn(bf2, "💾  Lưu File", self._save_cipher_file,
#                   bg="#004D40", fg="white", side=tk.LEFT)

#     # ---------- DECRYPT PANEL ----------
#     def _build_decrypt_panel(self, parent):
#         frame = self._panel(parent, "🔓  GIẢI MÃ DỮ LIỆU", "#FCE4EC", col=2)

#         tk.Label(frame, text="Bản mã (để giải mã):", bg="#FCE4EC",
#                  font=("Segoe UI", 10, "bold")).pack(anchor="w")
#         self.dec_input = tk.Text(frame, height=4, font=("Courier New", 9),
#                                  relief=tk.SOLID, bd=1, wrap=tk.WORD)
#         self.dec_input.pack(fill=tk.X, pady=4)

#         bf = tk.Frame(frame, bg="#FCE4EC")
#         bf.pack(fill=tk.X, pady=2)
#         self._btn(bf, "📂  Mở File", self._open_cipher_file, bg="#AD1457", fg="white", side=tk.LEFT)
#         self._btn(bf, "❌  Xóa", lambda: self.dec_input.delete("1.0", tk.END),
#                   bg="#B71C1C", fg="white", side=tk.LEFT)

#         self._btn(frame, "🔓  Giải mã", self._decrypt, bg="#AD1457", fg="white")

#         tk.Label(frame, text="Khóa bí mật (16 ký tự HEX):", bg="#FCE4EC",
#                  font=("Segoe UI", 10, "bold")).pack(anchor="w", pady=(6,0))
#         self.dec_key_display = tk.Text(frame, height=2, font=("Courier New", 11),
#                                        relief=tk.SOLID, bd=1, wrap=tk.WORD)
#         self.dec_key_display.pack(fill=tk.X, pady=2)

#         tk.Label(frame, text="Bản rõ (sau giải mã):", bg="#FCE4EC",
#                  font=("Segoe UI", 10, "bold")).pack(anchor="w", pady=(6,0))
#         self.dec_output = tk.Text(frame, height=4, font=("Segoe UI", 10),
#                                   relief=tk.SOLID, bd=1, wrap=tk.WORD, state=tk.DISABLED)
#         self.dec_output.pack(fill=tk.X, pady=4)

#         bf2 = tk.Frame(frame, bg="#FCE4EC")
#         bf2.pack(fill=tk.X, pady=2)
#         self._btn(bf2, "💾  Lưu File", self._save_plain_file, bg="#004D40", fg="white", side=tk.LEFT)
#         self._btn(bf2, "✔  Kiểm tra kết quả", self._verify_result,
#                   bg="#6A1B9A", fg="white", side=tk.LEFT)

#     # ---------- HELPERS ----------
#     def _panel(self, parent, title, bg, col):
#         outer = tk.Frame(parent, bg=bg, relief=tk.RAISED, bd=1)
#         outer.grid(row=0, column=col, sticky="nsew", padx=6, pady=4)
#         tk.Label(outer, text=title, font=("Segoe UI", 12, "bold"),
#                  bg=bg, fg="#1A237E", pady=8).pack(fill=tk.X)
#         ttk.Separator(outer, orient="horizontal").pack(fill=tk.X, padx=10)
#         inner = tk.Frame(outer, bg=bg, padx=12, pady=8)
#         inner.pack(fill=tk.BOTH, expand=True)
#         return inner

#     def _btn(self, parent, text, command, bg="#1976D2", fg="white", side=None):
#         b = tk.Button(parent, text=text, command=command, bg=bg, fg=fg,
#                       font=("Segoe UI", 9, "bold"), relief=tk.FLAT,
#                       padx=10, pady=6, cursor="hand2",
#                       activebackground=bg, activeforeground=fg)
#         if side:
#             b.pack(side=side, padx=2, pady=3)
#         else:
...
#             self.key_entry.config(state=tk.DISABLED, bg="#CFD8DC")
#             self._generate_key()
#         else:
